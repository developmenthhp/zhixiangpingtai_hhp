...
import com.zhixiangmain.Base.ObjectUtil;
...
import com.zhixiangmain.web.responseConfig.ResponseResult;
...
public class UserController {
	...
	public ResultBean setUser(@RequestParam("roleIds") String roleIds, @RequestParam("siteRoleIds") String siteRoleIds, User user) {
		...
		try {
			...
			if (ObjectUtil.isNull(user.getUsername())||ObjectUtil.isNull(user.getEmail())||ObjectUtil.isNull(user.getMobile())||ObjectUtil.isNull(user.getPassword())) {
				logger.debug("设置用户[新增或更新]，结果=请您填写用户信息");
				rb.setCode(IStatusMessage.SystemStatus.LOGIC_ERROR.getCode());//未通过校验
				rb.setMsg("请您填写用户信息");
			...
			String msg =  userService.setUser(user, roleIds, existUser, siteRoleIds);
			...
			if(!msg.equals("ok")){
				...
				rb.setCode(IStatusMessage.SystemStatus.SUCCESS.getCode());//未通过校验
				rb.setMsg(whichMsg);
			}
	...
	public Map<String, Object> getUserAndRoles(@RequestParam("id") Integer id) {
		...
		try {
			...
			if (null != urvo) {
				...
				List<Role> roles = this.authService.getRoles(sdId);
				logger.debug("查询角色数据！roles=" + roles);
				if (null != roles && roles.size() > 0) {
					map.put("roles", roles);
				}
				// 获取全部站点角色数据
				List<SiteRole> siteRoles = this.siteService.getSiteRoles(sdId);
