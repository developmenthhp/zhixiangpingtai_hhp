package com.zhixiangmain.filter;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.zhixiangmain.module.user.User;
import com.zhixiangmain.shiro.ShiroFilterUtils;
...
public class KickoutSessionFilter extends AccessControlFilter {
	...
	protected boolean onAccessDenied(ServletRequest request,
		...
		try {
			...
			logger.error(
					"控制用户在线数量【zhixiangyun-admin-->KickoutSessionFilter.onAccessDenied】异常！",
					e);
	...
	public static void out(ServletResponse response, ResponseResult result){
		...
		try {
			...
			logger.error("用户在线数量限制【zhixiangyun-admin-->KickoutSessionFilter.out】响应json信息成功");
		} catch (Exception e) {
			logger.error("用户在线数量限制【zhixiangyun-admin-->KickoutSessionFilter.out】响应json信息出错", e);
		}finally{
			if(null != out){
				out.flush();
				out.close();
