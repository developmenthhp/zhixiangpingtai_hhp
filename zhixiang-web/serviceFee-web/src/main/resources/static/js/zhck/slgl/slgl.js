...
$(function(){
    ...
    $("#liParent116").children(":eq(1)").children(":eq(2)").children(":eq(1)").children(":eq(0)").find("a").css("color","#FFD600");
    ...
    socket.on('tlAlarm', function(data) {
        /*var curJsonData = JSON.parse(data);
        $.each(curJsonData,function (ind,val) {
            if(allRabConfirmMsg!=undefined&&allRabConfirmMsg!=null&&allRabConfirmMsg.length>0){
                if($.inArray(val.sdId, allRabConfirmMsg)!=-1){
                    //当前在以前数组，不删除该节点

                }
            }else{
                allRabConfirmMsg.push(val.sdId);
                //将预警写入页面
                //$("#current_price").html(jsonObj.current_price);
            }
            console.log("收到cfd_md的消息："+val.sdId+",,,"+val.photo);

        });
        allRabConfirmMsg = curJsonData;*/
...
function loadMyLChar(rloadSdIds) {
    $.post("/threeLeaveAlarm/getTLAlarmPieBySdId",{"sdIds":rloadSdIds},function(data){
        if(data.code==1000){
            //刷新数据
            //站点三离报警次数排行
            $("#rankingList").html("");
