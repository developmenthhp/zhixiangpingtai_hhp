...
var ajaxSCJCXXSearchFormOption = {
    ...
    url: "/notarizedCertificate/getCirInfoBySdId", //默认是form的action， 如果申明，则会覆盖
    ...
    if(!isKickOut(data.code)){
        if(data.rows!=null){
            ...
            $.each(data.rows,function (ind,val) {
                ...
                var circulationCardImg = '';
                if(val.circulationCardImg){
                    circulationCardImg = val.circulationCardImg;
                }
                ...
                $("#userListUl").append("<tr class='myTableTooltip' title='"+siteName+":所属'>" +
                    ...
                    "</td>" +
                    "<td data-label='餐饮许可证' class='comMove'>" +
                    /*"<small id='"+val.id+"-ccjjTd' class='label label-danger' style='font-size: 90%'><i class='fa fa-list-ol'></i>&nbsp;"+val.inventoryLimit+""+val.meteringName+"</small>" +*/
                    "<div class='my-table-imgDiv'><img data-original='"+circulationCardImg+"' src='"+circulationCardImg+"' class='online' style='width:100%;cursor:zoom-in;'/></div>" +
                    "</td>" +
                    "</tr>");
            });
