...
import com.zhixiangmain.filter.KickoutSessionFilter;
import com.zhixiangmain.shiro.ShiroRealm;
import com.zhixiangmain.shiro.RetryLimitHashedCredentialsMatcher;
...
//		// 配置不会被拦截的链接 从上向下顺序判断
		...
		//filterMap.put("/swagger-ui.html", "anon"); filterMap.put("/swagger-resources/**", "anon"); filterMap.put("/v2/api-docs", "anon"); filterMap.put("/webjars/springfox-swagger-ui/**", "anon");


		filterChainDefinitionMap.put("/login", "anon");
		//这里先允许访问基础食材信息接口
		filterChainDefinitionMap.put("/bioFood/getBIOFoodList", "anon");
		//这里先允许访问基础食材信息添加接口
		/*filterChainDefinitionMap.put("/bioFood/setBasicInfoFood", "anon");
		filterChainDefinitionMap.put("/bioFood/getBasicInfoFoodByIds", "anon");*/
		//这里先允许访问基础菜品查询信息接口
		filterChainDefinitionMap.put("/foodBase/getFoodBaseList", "anon");
		//这里先允许访问基础菜品组成成分查询信息接口
		filterChainDefinitionMap.put("/foodIngredient/searchByOutFoodId", "anon");
		//这里先允许访问扫码查看食材使用信息
		filterChainDefinitionMap.put("/bioFood/getIngBUseBySdIdName", "anon");
		//这里先允许访问扫码查看菜品BOM信息
		filterChainDefinitionMap.put("/foodBase/getCompositionBySdIdId", "anon");
		//这里先允许访问扫码查看明厨亮灶信息
		filterChainDefinitionMap.put("/brightkitchen/getHFivePage", "anon");
		//放行获取该试点临期过期时间
		filterChainDefinitionMap.put("/site/isSiteOverTime", "anon");
...
//		// <!-- authc:所有url都必须认证通过才可以访问; anon:所有url都都可以匿名访问【放行】-->
	 ...
	 * 为实现session共享，使用下边redissessiondao
	 */
	@Bean
	...
	public DefaultWebSessionManager sessionManager() {
		...
		sessionManager.setGlobalSessionTimeout(redisSessionDao.getExpireTime());// 设置全局session超时时间
