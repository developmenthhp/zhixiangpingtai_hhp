package com.zhixiangmain.controller.zhjg.twjcgl;
 ...
 * @Package com.zhixiangyun.controller.zhjg.twjcgl
 * @Description: ${todo}
 * @author: hhp
 * @date: 2019-05-25 10:57
...
public class TemperatureController {
    ...
    public ModelAndView permList() {
        ...
        ModelAndView mav = new ModelAndView("/zhjg/twjcgl/twjcgl");
        try {
     ...
     * @return ModelAndView
     */
    @PostMapping( "/getTemperatureBySdId")
    @ResponseBody
    public ResultBean getTemperatureBySdId(TemperatureDTO temperatureDTO) {
        logger.debug("平台对传过来的站点进行标准穿戴管理分析！查询条件：temperatureDTO：" + temperatureDTO);
        ResultBean resultBean = new ResultBean();
        User existUser= (User) SecurityUtils.getSubject().getPrincipal();
        temperatureDTO.setUserId(existUser.getId());
        String path = TemperatureController.class.getClassLoader().getResource("json/siteData.json").getPath();
        String siteJsonData = JsonUtil.readJsonFile(path);
        JSONObject jobj = JSON.parseObject(siteJsonData);
        try {
            resultBean = temperatureService.getTemperatureBySdId(temperatureDTO,jobj);
        } catch (Exception e) {
            e.printStackTrace();
