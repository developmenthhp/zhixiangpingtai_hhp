package com.zhixiangmain.impl.inventory;
...
import com.zhixiangmain.api.service.inventory.InventoryAlertService;
 ...
 * @Description: ${todo}
 * @author: hhp
 * @date: 2019-05-31 13:12
 * @Copyright: 2019 www.zhixiangyun.net Inc. All rights reserved.
 * 注意：本内容仅限于浙江智飨科技内部传阅，禁止外泄以及用于其他的商业目
 */
@Service(version = "1.0.0", interfaceClass = InventoryAlertService.class)
public class InventoryAlertServiceImpl implements InventoryAlertService {
    private static final Logger logger = LoggerFactory
            .getLogger(InventoryAlertServiceImpl.class);
    @Autowired
    ...
    @Override
    public ResultBean getIBMMonthBySdId(InventoryAlertDTO inventoryAlertDTO, JSONObject jobj) {
        ResultBean resultBean = new ResultBean();
        List<Map<String,Object>> temperatureList = Lists.newArrayList();
        if(!IsEmptyUtils.isEmpty(inventoryAlertDTO.getSdIds())){
            String[] sdIds = inventoryAlertDTO.getSdIds().split(",");
            for(String sdId:sdIds){
                ...
                if(!IsEmptyUtils.isEmpty(dataSourceName)){
                    ...
                    inventoryAlertDTO.setSdId(Integer.parseInt(sdId));
                    //食材临期月数据
                    List<Map<String,Object>> temperatureMap  = inventoryAlertMapper.findIBMMonthBySdId(inventoryAlertDTO);
            ...
            //查询该用户所拥有的站点权限
            List<SiteVO> userSites = siteMapper.findUserSites(inventoryAlertDTO.getUserId());
            for(SiteVO siteVO:userSites){
                ...
                if(!IsEmptyUtils.isEmpty(jobj)){
                    ...
                    if(!IsEmptyUtils.isEmpty(dataSourceName)){
                        ...
                        DynamicDataSourceContextHolder.setDataSourceType(dataSourceName);
                        //获取采购单
                        inventoryAlertDTO.setSdId(siteVO.getSdId());
                        //食材临期月数据
                        List<Map<String,Object>> temperatureMap  = inventoryAlertMapper.findIBMMonthBySdId(inventoryAlertDTO);
                        temperatureList = getMonthDataList(temperatureList,temperatureMap);
                    }
