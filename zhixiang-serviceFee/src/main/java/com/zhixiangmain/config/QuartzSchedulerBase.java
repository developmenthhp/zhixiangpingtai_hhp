...
import com.zhixiangmain.module.base.ServiceTimeJob;
import org.quartz.*;
...
public class QuartzSchedulerBase {
     ...
     * */
    public void startSTimeJob() throws SchedulerException {
        startServiceTimeJob(scheduler);
        scheduler.start();
    }
    /**
     * 获取Job信息
   ...
   /**
     ...
     * */
     ...
     private void startServiceTimeJob(Scheduler scheduler) throws SchedulerException {
         // 通过JobBuilder构建JobDetail实例，JobDetail规定只能是实现Job接口的实例
         // JobDetail 是具体Job实例
         JobDetail jobDetail = JobBuilder.newJob(ServiceTimeJob.class).withIdentity("serviceFee", "serviceFee").build();
         // 基于表达式构建触发器
         CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule("0/10 * * * * ?");//每10秒执行一次
         // CronTrigger表达式触发器 继承于Trigger
         // TriggerBuilder 用于构建触发器实例
         CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity("serviceFee", "serviceFee") .withSchedule(cronScheduleBuilder).build(); scheduler.scheduleJob(jobDetail, cronTrigger);
     }
